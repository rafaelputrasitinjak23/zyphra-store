...
const { requirementsFor } = require('../services/authPolicy');
...
test('OTP hanya dapat digunakan sekali', () => {
  ...
  assert.equal(isOtpUsable(otp), true); otp.consumedAt = new Date(); assert.equal(isOtpUsable(otp), false);
});
...
test('harga dari frontend diabaikan dan harga database digunakan', () => {
  ...
  assert.equal(row.unitPrice, 20000); assert.equal(row.lineTotal, 40000);
});
test('OAuth tidak meminta OTP/CAPTCHA sedangkan login manual selalu meminta keduanya', () => {
  assert.deepEqual(requirementsFor('manual'), { captcha: true, password: true, otp: true });
  assert.deepEqual(requirementsFor('google'), { captcha: false, password: false, otp: false });
  assert.deepEqual(requirementsFor('github'), { captcha: false, password: false, otp: false });
});
