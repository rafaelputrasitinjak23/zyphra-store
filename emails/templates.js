...
function invoiceTemplate({ order, actionUrl }) {
  ...
  const walletRow = order.walletAmount > 0 ? `<tr><td style="padding:8px">Saldo Zyphra</td><td style="text-align:right">${rupiah(order.walletAmount)}</td></tr>` : '';
  const feeRow = order.userFee > 0 ? `<tr><td style="padding:8px">Biaya layanan</td><td style="text-align:right">${rupiah(order.userFee)}</td></tr>` : '';
  const itemsSubtotal = order.itemsSubtotal || (order.subtotal + (order.discountAmount || 0));
  return shell(`Invoice ${order.invoiceNumber}`, `<p>Nomor pesanan: <strong>${order.orderNumber}</strong></p><table style="width:100%;border-collapse:collapse">${rows}<tr><td style="padding:8px">Subtotal produk</td><td style="text-align:right">${rupiah(itemsSubtotal)}</td></tr>${discountRow}${walletRow}${feeRow}<tr><td style="padding:8px;font-weight:bold">Total transaksi</td><td style="text-align:right;font-weight:bold">${order.total === 0 ? 'Gratis' : rupiah(order.total)}</td></tr><tr><td style="padding:8px">Status</td><td style="text-align:right">${order.paymentStatus}</td></tr></table>${actionUrl ? button('Lihat pesanan', actionUrl) : ''}`);
}
